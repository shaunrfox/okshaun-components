...
<Meta title="Docs / Build Reasoning" />
...
- Moved `cetec-preset.ts` --> `src/cetec-preset.ts` to keep the preset alongside other source exports and align Vite entry resolution.
...
- We export `cetecPreset` as `any` to avoid Panda type identity conflicts in Astro
...
<Grid gridTemplateColumns="auto 1fr" gap="2" mb="32" rounded="16" overflow="hidden" className="definitions-grid">
  ...
  <Text as="p" gridColumn="1 / -1" textStyle="mono.lg" color="text">npm run build</Text>
...
<Text as="p" gridColumn="1 / -1" textStyle="mono.lg" color="text">npm run panda-mcp</Text>
