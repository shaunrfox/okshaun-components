...
function resolveSemanticToken(token: ColorToken): { light: string; dark: string; lightRef: string; darkRef: string } {
  ...
  if (current && typeof current === 'object' && 'value' in current) {
    ...
    if (value && typeof value === 'object') {
      ...
      const darkValue = resolveColorReference(value._dark || value.base || value);
      return { light: lightValue, dark: darkValue, lightRef: lightRef, darkRef: darkRef };
    }
  }
...
function extractColorReference(ref: any): string {
...
function resolveColorReference(ref: any): string {
  if (typeof ref === 'string') {
    // Check if it's a reference like "{colors.neutral.20}"
    const match = ref.match(/^\{colors\.(.+)\}$/);
    if (match && match[1]) {
